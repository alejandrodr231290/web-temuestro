$(document).ready(function () {
    ...
    tabla.on('search.dt', function () {
        var sum = tabla.api().column(3, { search: 'applied' }).data().sum();
        var cobrado = tabla.api().column(4, { search: 'applied' }).data().sum();
        $('#total').text('$'+$.number(sum, 2, '.', ','));
        $('#cobrado').text('$'+$.number(cobrado, 2, '.', ','));
    });
    cargartabla();
    ...
    function cargartabla() {
        ...
        $('#tabla').dataTable({
            ...
            columns: [
                ...
                { data: "porcobrar", render: $.fn.dataTable.render.number(',', '.', 2, '$') }
            ],
            order: [[1, "desc"]],
            columnDefs: [
